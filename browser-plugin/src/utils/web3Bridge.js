...
// src/utils/localChainAnchor.source.js
var LOCAL_DEV_PRIVATE_KEY = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
...
async function ensureLocalChainReady() {
...
function isNonceError(error) {
  ...
  return error?.code === "NONCE_EXPIRED" || /nonce too low/i.test(message) || /nonce has already been used/i.test(message);
}
...
async function anchorViaLocalRpc(stateHashHex) {
  let releaseLock;
...
// src/utils/web3Bridge.source.js
async function anchorSessionToLedger(stateHashHex, platformId, timestamp, _tabId) {
  if (!NINK_CHAIN_CONFIG2.registryAddress || NINK_CHAIN_CONFIG2.registryAddress === "0x0000000000000000000000000000000000000000") {
    throw new Error(
  ...
  return {
    ...
    registryAddress: walletResult.registryAddress || NINK_CHAIN_CONFIG2.registryAddress,
