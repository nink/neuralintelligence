...
// src/utils/web3Bridge.source.js
import {
  NINK_CHAIN_CONFIG as NINK_CHAIN_CONFIG3,
  NINK_REGISTRY_ABI as NINK_REGISTRY_ABI2,
  NINK_TOKEN_ABI as NINK_TOKEN_ABI2
} from "../config/chainConfig.js";

// src/utils/localChainAnchor.source.js
import {
  NINK_CHAIN_CONFIG,
  NINK_REGISTRY_ABI,
  NINK_TOKEN_ABI
...
// src/utils/stateHash.js
function normalizeStateHashHex(stateHashHex) {
  const normalized = String(stateHashHex || "").trim().replace(/^0x/i, "").toLowerCase();
...
// src/utils/localChainAnchor.source.js
var LOCAL_HARDHAT_CHAIN_ID = 31337;
var anchorLock = Promise.resolve();
function isLocalHardhatChain() {
  return Number(NINK_CHAIN_CONFIG.chainId) === LOCAL_HARDHAT_CHAIN_ID;
}

// src/utils/walletInject.js
var WALLET_ANCHOR_SCRIPT = "src/inpage/walletAnchor.js";
...
async function anchorViaActiveTab(tabId, injectionArgs) {
  if (!tabId) {
...
// src/utils/tokenBalance.js
import { NINK_CHAIN_CONFIG as NINK_CHAIN_CONFIG2 } from "../config/chainConfig.js";
var LOCAL_HARDHAT_CHAIN_ID2 = 31337;
var LOCAL_RPC_URL = "http://127.0.0.1:8545";
...
function resolveRpcUrl() {
  if (Number(NINK_CHAIN_CONFIG2.chainId) === LOCAL_HARDHAT_CHAIN_ID2) {
    return LOCAL_RPC_URL;
  }
  return NINK_CHAIN_CONFIG2.rpcUrl || "";
}
async function readNinkAllowance(ownerAddress, spenderAddress, rpcUrl = resolveRpcUrl()) {
  if (!NINK_CHAIN_CONFIG2.tokenAddress || !ownerAddress || !spenderAddress || !rpcUrl) {
    return null;
  }
  const data = `0xdd62ed3e${padAddress(ownerAddress)}${padAddress(spenderAddress)}`;
  const result = await rpcCall(rpcUrl, "eth_call", [
    { to: NINK_CHAIN_CONFIG2.tokenAddress, data },
    "latest"
  ]);
  return decodeUint256(result);
}
async function readAnchorFee(rpcUrl = resolveRpcUrl()) {
  if (!NINK_CHAIN_CONFIG2.registryAddress || !rpcUrl) {
    return null;
  }
  const data = "0x919acbe6";
  const result = await rpcCall(rpcUrl, "eth_call", [
    { to: NINK_CHAIN_CONFIG2.registryAddress, data },
    "latest"
  ]);
  return decodeUint256(result);
}

// src/utils/platformIds.js
var PLATFORM_HOST_IDS = {
  "chatgpt.com": 1,
  "claude.ai": 2,
  "gemini.google.com": 3,
  "grok.com": 4,
...
function resolvePlatformIdFromTab(tabUrl, sourcePlatform = "") {
  try {
...
async function anchorViaMetaMask(tabId, stateHash, connectedAddress) {
  ...
  const allowance = await readNinkAllowance(connectedAddress, NINK_CHAIN_CONFIG3.registryAddress) ?? 0n;
  const tokenIface = new Interface(NINK_TOKEN_ABI2);
  const registryIface = new Interface(NINK_REGISTRY_ABI2);
  const injectionArgs = {
    expectedChainId: Number(NINK_CHAIN_CONFIG3.chainId),
    tokenAddress: NINK_CHAIN_CONFIG3.tokenAddress,
    registryAddress: NINK_CHAIN_CONFIG3.registryAddress,
    anchorCallData: registryIface.encodeFunctionData("anchorState", [stateHash]),
    approveCallData: allowance < anchorFee ? tokenIface.encodeFunctionData("approve", [
      NINK_CHAIN_CONFIG3.registryAddress,
      MaxUint256
    ]) : null
  };
  const walletResult = await anchorViaActiveTab(tabId, injectionArgs);
  const transactionHash = walletResult?.transactionHash && String(walletResult.transactionHash).trim();
  if (!transactionHash || transactionHash === "null") {
    throw new Error(
      "Anchor transaction completed without a transaction hash. Wallet response: " + JSON.stringify(walletResult)
    );
  }
  return {
    transactionHash,
    ...
    registryAddress: walletResult.registryAddress || NINK_CHAIN_CONFIG3.registryAddress,
    chainId: walletResult.chainId ?? Number(NINK_CHAIN_CONFIG3.chainId),
    anchorMethod: "metamask-tab",
    anchorFeePaid: anchorFee.toString()
  };
}
async function anchorSessionToLedger(stateHashHex, platformId, timestamp, tabId, options = {}) {
  if (!NINK_CHAIN_CONFIG3.registryAddress || NINK_CHAIN_CONFIG3.registryAddress === "0x0000000000000000000000000000000000000000") {
    throw new Error(
